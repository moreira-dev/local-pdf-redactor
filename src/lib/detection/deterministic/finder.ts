...
const rules: Rule[] = [
	 ...
	 */
	{ category: 'BSB', pattern: /(?<=\bBSB\b[:#\s]*)\d{3}[\s-]?\d{3}(?!\d)/gi },

	/**
	 * BSB numbers hyphenated.
	 * Matches: '062-123', '999-999'
	 */
	{ category: 'BSB', pattern: /(?<!\d)\d{3}-\d{3}(?!\d)/g },
	 ...
	 */
	{ category: 'DATE_OF_BIRTH', pattern: new RegExp(`(?<=\\b${birthLabel}[:\\s]*)${date}`, 'gi') },
];
