import type { PIIDetection, TextLine } from '$lib/types';
...
/**
 * Matches months in abbreviated or full forms, optionally ending with a full stop.
...
const rules: Rule[] = [
	 ...
	 */
	{ label: 'BSB', pattern: /(?<=\bBSB\b[:#\s]*)\d{3}[\s-]?\d{3}(?!\d)/gi },

	/**
	 * BSB numbers hyphenated.
	 * Matches: '062-123', '999-999'
	 */
	{ label: 'BSB', pattern: /(?<!\d)\d{3}-\d{3}(?!\d)/g },
	 ...
	 */
	{ label: 'DATE_OF_BIRTH', pattern: new RegExp(`(?<=\\b${birthLabel}[:\\s]*)${date}`, 'gi') },
];

/**
 * Checks if we already detected PII in the given string range so that we don't duplicate it.
 */
function isAlreadyFound(foundPIIList: PIIDetection[], start: number, end: number): boolean {
	for (const detection of foundPIIList) {
...
function findPIIInLine(line: TextLine, lineIndex: number): PIIDetection[] {
	const foundPIIList: PIIDetection[] = [];
	...
	for (const rule of rules) {
		for (const match of line.text.matchAll(rule.pattern)) {
			...
			if (isAlreadyFound(foundPIIList, start, end)) {
				continue;
			}
	...
	foundPIIList.sort((left, right) => left.start - right.start);
 ...
 */
export function findPIIByDeterministicRules(lines: TextLine[]): PIIDetection[] {
